...
class PilePromptDataset(Dataset):
    """Dataset for generating prompts from The Pile."""
    ...
    def __init__(self, num_prompts: int = 50000, sequence_length: int = 512):
        ...
        self.dataset = load_dataset("EleutherAI/pile", split="train", streaming=True, trust_remote_code=True)
...
class OpenSAEExtractor:
    """Extract activations from an open-access model."""
    ...
    def __init__(self, 
        ...
        """
        Initialize the open SAE extractor.
    ...
    def extract_heldout_codes(self, 
        ...
        dataset = PilePromptDataset(num_prompts=num_prompts, sequence_length=sequence_length)
...
def main():
    ...
    # Initialize extractor with an open-access model
    extractor = OpenSAEExtractor(
    ...
    heldout_codes = extractor.extract_heldout_codes(
        num_prompts=50000,  # Full 50K prompts
        batch_size=32,
        sequence_length=512
    )
